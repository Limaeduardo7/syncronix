...
const DEFAULT_WORDS = ["SYNCRONIX", "ORIGINAL", "LIFESTYLE"]
...
export function ParticleTextEffect({ 
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Generate new color - Using purple/primary theme
    const newColor = {
      r: 169 + Math.random() * 50, // Purple tones
      g: 0 + Math.random() * 100,
      b: 255,
    }
  ...
  const animate = () => {
    ...
    if (autoAdvance) {
      ...
      const framesPerInterval = (intervalMs / 1000) * 60 // Assuming 60 FPS
      if (frameCountRef.current % framesPerInterval === 0) {
  ...
  }, [words, width, height, fontSize, autoAdvance, intervalMs])
