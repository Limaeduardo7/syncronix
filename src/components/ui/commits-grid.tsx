...
export const CommitsGrid = ({ text }: { text: string }) => {
  ...
  return (
    <section
      className="w-full max-w-4xl bg-card border grid p-3 sm:p-6 gap-1 sm:gap-2 rounded-[15px] sm:rounded-[20px]"
      style={{
      ...
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={cn(
              `border h-full w-full aspect-square rounded-[6px] sm:rounded-[8px] min-h-[8px] min-w-[8px] sm:min-h-[12px] sm:min-w-[12px]`,
